...
ring_buffer rx_buffer = { { 0 }, 0, 0}; // notation to initialize struct with specific values
...
{
  _rx_buffer = &rx_buffer; // ?? head?
  _tx_buffer = &tx_buffer; // ?? tail?
...
//	// force add something into rxBuffer
//	_rx_buffer->buffer[0] = 'b'; // write 'b' into rxBuffer
//	_rx_buffer->head += 1; // increment head after write
...
void Uart_write(int c)
{
	if (c>=0)
	{
		...
		// output buffer is full when head is one less than tail
		// If the output buffer is full, there's nothing for it other than to
		// wait for the interrupt handler to empty it a bit
		// ???: return 0 here instead?
		while (i == _tx_buffer->tail);

		_tx_buffer->buffer[_tx_buffer->head] = (uint8_t)c; // write c into buffer
		_tx_buffer->head = i; // increment head
...
//	  uint32_t cr1its     = READ_REG(huart->Instance->CR1);
	  ...
	  /* change everything to point at huart2 explicitly */
//	  uint32_t isrflags   = READ_REG((&huart2)->Instance->ISR);
//	  uint32_t cr1its     = READ_REG((&huart2)->Instance->CR1);

    /* if DR is not empty and the Rx Int is enabled */
	  // USART_CR1_RXNEIE = 0x20, USART_CR1_RXNEIE = 0x20
//    if (((isrflags & USART_ISR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET))
	// directly read from huart2
...
//    if (((isrflags & USART_ISR_TXE) != RESET) && ((cr1its & USART_CR1_TXEIE) != RESET))
    if ((((&huart2)->Instance->ISR & USART_ISR_TXE) != RESET) && (((&huart2)->Instance->CR1 & USART_CR1_TXEIE) != RESET))
    {
...
//    	      unsigned char c = tx_buffer.buffer[tx_buffer.tail]; // unsigned char -- FAIL
    		  volatile unsigned int tail_test = tx_buffer.tail;
    		  volatile uint16_t c = tx_buffer.buffer[tx_buffer.tail]; // prevent optimized away
//    	      uint8_t c = tx_buffer.buffer[tx_buffer.tail-1]; // uint8_t -- FAIL
...
//    	      huart->Instance->RDR = c;
    	      ...
    	      (&huart2)->Instance->ISR; // ?? dont't know why this is needed
//    	      (&huart2)->Instance->RDR = c; // receive data register not being set to value of c
    	      (&huart2)->Instance->TDR = c; // transmit data register
